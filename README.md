...
1. Получите `Manyasha-0.1.0-macos.zip` и соседний файл контрольной суммы.
...
Пакет macOS сейчас является проверяемым кандидатом 0.1.0: он прошёл установку из распакованного архива на Mac, но ещё не подписан Apple Developer ID и не опубликован как production-релиз. Windows остаётся обязательной следующей целью и не обозначается готовой.
...
Проверка локальной модели, мессенджеров, Windows-пакета и полной OS-песочницы ещё не выполнена.
...
- Публичный GitHub remote не создан: владелец/организация и имя репозитория ещё не подтверждены.
