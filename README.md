...
Пакет macOS сейчас является проверяемым кандидатом 0.2.0: собственный GUI начинает работу с разговора, три стартовых действия заполняют новую задачу, ответ API.Navy сохраняется в том же локальном профиле, который читает терминальная команда `manyasha sessions`. Пакет ещё не подписан Apple Developer ID и не опубликован как production-релиз. Windows остаётся обязательной следующей целью и не обозначается готовой.
